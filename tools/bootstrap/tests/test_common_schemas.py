...
class CommonSchemasContractTest(unittest.TestCase):
    ...
    def test_timestamp_schema_structure(self):
        ...
        self.assertFalse(data.get("additionalProperties"))
    ...
    def test_provenance_schema_structure(self):
        ...
        self.assertEqual(set(data["properties"]["provenance_type"]["enum"]), {"LIVE", "REPLAY", "SYNTHETIC"})
    ...
    def test_evidence_envelope_schema_structure(self):
        ...
        self.assertEqual(data.get("type"), "object")
        self.assertFalse(data.get("additionalProperties"))
        self.assertEqual(set(data.get("required", [])), {"schema_version", "payload_type", "timestamp", "provenance", "payload"})
    ...
    def test_common_envelope_fixture_validates(self):
        ...
        self.assertEqual(fixture["provenance"]["provenance_type"], "SYNTHETIC")
        ...
        try:
            ...
            # Validate fixture against envelope schema using a registry or embedding
            schema_store = {
            ...
            if hasattr(jsonschema, "validators") and hasattr(jsonschema.validators, "Draft202012Validator"):
                from jsonschema import Registry, Resource
                registry = Registry().with_resources([
                    ("timestamp_v1.schema.json", Resource.from_contents(timestamp_schema)),
                    ("provenance_v1.schema.json", Resource.from_contents(provenance_schema)),
                    (timestamp_schema["$id"], Resource.from_contents(timestamp_schema)),
                    (provenance_schema["$id"], Resource.from_contents(provenance_schema)),
                ])
